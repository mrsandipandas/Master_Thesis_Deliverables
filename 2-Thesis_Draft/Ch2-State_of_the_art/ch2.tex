...
\end{figure}
	...
	\subsection{Coupling mode theory}
	\todo{Include the mathematical expressions from ch 5(Microphotonica) on couplers}
	
	\subsection{Ring resonator}
	\todo{Write it because used in Berry's phase}
...
\gls{pr} are realized in different ways for optical fiber and \gls{oeic} which are discussed in this section.	
...
The fields in \gls{te} and \gls{tm} mode do not couple with each other and are of different symmetry. Thus asymmetric structure in both horizontal and vertical directions are required to break the symmetry which are accomplished using different principles viz. mode coupling \cite{dai_novel_2011,ding_Integrated_2013,wang_design_2014}, mode evolution \cite{zhang_selected_2010,chen_compact_2011,zhang_efficient_2012,justin_conference_2012,kazuhiro_integrated_2015} and mode hybridization \cite{fukuda_integrated_2008,leung_numerical_2011,vermeulen_Silicon_2012,velasco_ultracompact_2012}, described in the following sections.
			...
			\paragraph*{Mode coupling:} Mode coupling \gls{pr} includes a pair of waveguide core layers running parallel to each other, with a small coupling region. When two modes with orthogonal polarizations have equal effective \gls{ri}s, strong mode coupling occurs in the waveguide, and with proper taper orientation and design, length of coupling region, one mode can be effectively converted to the other.
...
\begin{itemize}[leftmargin=*]
	...
	\item[$\square$] \begin{minipage}[t]{\textwidth}\textbf{Design: Mode evolution \gls{pr} based on single taper}
	\begin{figure}[H] %h
		\begin{subfigure}[t]{0.45\textwidth}
	...
	\item[$\square$] \begin{minipage}[t]{\textwidth}\textbf{Design: Mode evolution \gls{pr} composed of an asymmetric-rib waveguide and a tapered waveguide}
	\begin{figure}[H] %h
		\begin{subfigure}[t]{0.45\textwidth}
...
\begin{itemize}[leftmargin=*]
	...
	\item[$\square$] \begin{minipage}[t]{\textwidth}\textbf{Design: Asymmetric silicon nanowire waveguide as compact \gls{pr}}
	\begin{figure}[H] %h
		\begin{subfigure}[t]{0.45\textwidth}
