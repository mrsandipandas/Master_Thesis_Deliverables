...
Light can travel through dielectric waveguide. To comprehend the situation we need to look at physical properties of light and waveguides. The basic understanding of Maxwell's equations are required to combine the electric fields and magnetic fields to combine into a wave equation. Since, the optical system changes polarization we need a formal definition of the different \gls{sop}, described using Jones calculus. Moreover, the transition from one \gls{sop} to another, can be defined formally on the Poincaré sphere. 
		
		\subsection{Maxwell's equations}
\gls{em} radiation is the radiant energy released by varying \gls{em} field. Light is \gls{em} wave at very high frequency. The frequency of visible light falls in between IR and UV \gls{em} wave. 
\begin{figure}[h]
	\centering
...
The currently available \gls{ic} \gls{pr}s can be classified under two categories as passive and active \gls{pr}. In the passive \gls{pr}s the waveguide structures are designed in a specific way to manipulate the effective \gls{ri} of the waveguide to obtain the desired polarization. The structures cannot be manipulated or tuned once fabricated. Whereas, in active \gls{pr}s, the effective \gls{ri} is manipulated by applying voltage to a fixed waveguide structure and using quantum optical phenomenon.   
	
		\subsubsection{Passive PR}
The currently, available passive \gls{pr}s are based on the principles of mode evolution \cite{zhang_selected_2010,chen_compact_2011,zhang_efficient_2012,justin_conference_2012,kazuhiro_integrated_2015}, mode coupling \cite{dai_novel_2011,ding_Integrated_2013,wang_design_2014} and mode hybridization \cite{fukuda_integrated_2008,leung_numerical_2011,vermeulen_Silicon_2012,velasco_ultracompact_2012}.
		...
		\subsubsection{Comparative analysis of available polarization rotator}
\end{document}
